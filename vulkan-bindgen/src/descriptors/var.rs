...
fn parse_c_type(code: &str) -> VarDescriptor {
    ...
    let delim = code.find(|c| c == ' ' || c == '*').expect("Could not get delim after type identifier");
    ...
    let delim = code.find(|c| c == '[' || c == ':' || c == ' ').unwrap_or(code.len());
    ...
    loop {
        ...
        let delim = code.find(|c| c == ']').expect("Missing closing array brace");
